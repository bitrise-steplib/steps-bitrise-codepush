...
// (internal/codepush/push_test.go). TestResolveDeployment landed in an earlier PR in this stack
// (see resolve_test.go); this file covers the rest of push_test.go — TestPush,
// TestValidatePushOptions, TestPollStatus. testOut now backs the local Logger interface instead of
// output.NewTest. Zip cleanup is no longer performed after Push (see push.go's doc comment); the
