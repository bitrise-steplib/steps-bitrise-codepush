// Package codepush ports the CodePush API push workflow (auth, upload-URL request
// construction, app/deployment resolution, rollout-percentage encoding) from
...
// (internal/codepush/types.go). This is a deliberate source copy, not a module dependency; see
...
//
// Only the push path was ported (Phase 1 scope). Rollback, promote, patch, and deployment
// management (create/rename/delete) were intentionally left out: they are Phase 2 scope or
// explicitly out of scope per the project brief. ListDeployments was kept because push needs it
// to resolve a deployment name to its ID.
package codepush
